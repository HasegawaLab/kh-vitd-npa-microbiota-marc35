...
# Vitamin-D related compounds            
   ...
   # LL-37
        summary(metadata_pam$LL37_ngml)   
                metadata_pam$LL37_ngml2 = 
                    ifelse(metadata_pam$LL37_ngml <46,  "Low (<46 ng/ml)",
                    ifelse(metadata_pam$LL37_ngml>=46, "High (>=46 ng/ml)",
                    NA))
            metadata_pam$LL37_ngm2 <- as.factor(metadata_pam$LL37_ngml2)
            table(metadata_pam$LL37_ngm2, useNA="ifany"); class(metadata_pam$LL37_ngml2)  
```
...
```{r Table 1, eval=TRUE}
    ...
    table1<-CreateTableOne(vars = vars, strata = c("vitd25_ngml_cat2"), 
                           ...
                           factorVars = c("female", "raceethn", 
                                         ...
                                         "vitd25_ngml_cat2", "bioD2", "freeD2", "LL37_ngml2"
                                         ))
          print(table1, nonnormal = c("Age_mo", "intch_weight","LOS_dys",
                                       "intch_respiratory_rate",
                                      "RANTES_pgml","LL37_ngml", "vitd25_ngml", "bioD", "freeD", "dbp_ugml", "LL37_ngml",
...
#7 Interactions between 25OHD and microbiota with regard to severity
## As random-effect model for the interactions does not converge -- fixed glm fitted.
```{r Interactions}
table1_tb$PAM4_m35 <- as.factor(table1_tb$PAM4_m35)
table1_tb$PAM4_m35 <- relevel (table1_tb$PAM4_m35, "Moraxella dominant") 

# No interaction model
  model_no_int <- glm (IntensiveTreatment ~ PAM4_m35
                         + Age_mo + female  + raceethn 
                         + premature37
                         # + mata_delivery
                         + prev_breathingprob2
                         + children_home2 
                         + breastmilk_mostly_3mo9 
                         # +intake_daycare  #+ intch_weight_cat2 
                         #+ antibiotics_life
                         + intch_antibiotics_ed 
                         + RSV + RV 
                         + LL37_ngml,  # LL-37 may be an intermediate of vitD-severity association
                         family=binomial, 
                         data=table1_tb)
# Interaction model
  model_int <- glm (IntensiveTreatment ~ PAM4_m35
                                    + PAM4_m35:vitd25_ngml
                                    + vitd25_ngml
                         + Age_mo + female  + raceethn 
                         + premature37
                         # + mata_delivery
...
# Calculate LR
  G2 = -2 * logLik(model_no_int) + 2 * logLik(model_int)
  pchisq(as.numeric(G2), df=3, lower.tail=F)  # If we use 25OHD as a binomal variable, df would become 7 (= reduced power).
```


#7-1. Stratification -- low total 25OHD stratum -- Significant associations b/w microbiota profile and severity
```{r Low 25OHD stratum}
# Data limited to LOW LL-RANTES group
   low_tbl <- filter(table1_tb, vitd25_ngml_cat2=="Low (<26.5 ng/ml)")
   dim(low_tbl)  

# Logistric regression
library("lme4")
    mdl_random <- glmer (IntensiveTreatment ~ PAM4_m35
                         + Age_mo + female  + raceethn 
                         + premature37
                         # + mata_delivery
                         + prev_breathingprob2
                         + children_home2 
                         + breastmilk_mostly_3mo9 
                         # +intake_daycare  #+ intch_weight_cat2     # Wt is has collinearity with age.
                         #+ antibiotics_life
                         + intch_antibiotics_ed 
                         + RSV + RV 
                         + LL37_ngml         
                ...
                #print(mdl_random, corr=FALSE)  
                      summary(mdl_random) #get p value
                #Estimate 95%CI
                      se <- sqrt(diag(vcov(mdl_random)))
                # table of estimates with 95% CI
                      (ORCI <- cbind(Est = fixef(mdl_random), 
                                     LL = fixef(mdl_random) - 1.96 * se, 
                                     UL = fixef(mdl_random) + 1.96 * se))
                      exp(ORCI)

```

#7-2. Stratification -- High total 25OHD stratum -- NO significant associations b/w microbiota profile and severity
```{r High 25OHD straturm}
# Data limited to LOW LL-RANTES group
   high_tbl <- filter(table1_tb, vitd25_ngml_cat2 != "Low (<26.5 ng/ml)")
   dim(high_tbl)  
...
## 8-0. Prepation for the analysis using MixOmics 
```{r Prep for MixOmics}
...
## 16.  Random forest, total 25OHD as binary 
```{r random forrest for binary}
# First unload mixOmics and upload dplyr to use the "select" function
      detach("package:mixOmics", unload=TRUE)
      detach("package:dplyr",    unload=TRUE)
      library(dplyr);
...
# Make rf dataset
  ...
  microbiota_tbl_rf_cat <- metadata_pam %>% 
                            select(SampleID_NPA, vitd25_ngml_cat2) %>%
                            inner_join(., microbiota_tbl_rf, by="SampleID_NPA") %>%
                            select(-SampleID_NPA)
  rownames(microbiota_tbl_rf_cat) <- rownames(microbiota_tbl)
...
# Fit random forrest  
  set.seed(1)
  rf_fit_cat <- train(vitd25_ngml_cat2 ~.,  # by specifing ".", enter all genera
                  data=microbiota_tbl_rf_cat,
                  method="rf", 
...
# Check the variable importance 
  Imp_variables_cat <- varImp(rf_fit_cat, scale=FALSE, 20)
  Imp_variables_cat_df <- as.data.frame(Imp_variables_cat[1])
  Imp_variables_cat_df$variable_rf_cat <- rownames(Imp_variables_cat_df)
# Create a vector of the top 20 genera
  head(Imp_variables_cat_df)
  
  rf_cat_top20 <- Imp_variables_cat_df %>% arrange(desc(importance.High....26.5.ng.ml.)) %>% .[1:20, "variable_rf_cat"]
  rf_cat_top20
```

## 17.  Random forest, total 25OHD as continuous variable
```{r random forrest, 25OHD as continuous}
#Dataset
  microbiota_tbl_rf_cont <- metadata_pam %>% 
                            select(SampleID_NPA, vitd25_ngml) %>%
                            inner_join(., microbiota_tbl_rf, by="SampleID_NPA") %>%
                            select(-SampleID_NPA)
  rownames(microbiota_tbl_rf_cont) <- rownames(microbiota_tbl)
  
# Fit random forrest  
  set.seed(1)
  rf_fit_cont <- train(vitd25_ngml ~.,  # by specifing ".", enter all genera
                  data=microbiota_tbl_rf_cont,
                  method="rf", 
                  #preProcess = c('center', 'scale'),
                  tuneLength = 3,  # Try 3 different values for each parameter
                  trControl = trainControl(method = "cv", number = 10), # 10-fold Cross validation
...
# Check the variable importance 
  Imp_variables_cont <- varImp(rf_fit_cont, scale=FALSE, 20)
  Imp_variables_cont_df <- as.data.frame(Imp_variables_cont[1])
  Imp_variables_cont_df$variable_rf_cont <- rownames(Imp_variables_cont_df)
# Create a vector of the top 20 genera
  rf_cont_top20 <- Imp_variables_cont_df %>% arrange(desc(Overall)) %>% .[1:20, "variable_rf_cont"]
  rf_cont_top20
```
...
## 18. Regression models: Here we examine the association between the each of the 20 selected genera and the risk of severity, the clincial outcome.  
```{r prep for regression}
# Integrate the datasets (genera abundance + metadata)
  ## rf -- categorical
    #colnames(microbiota_tbl_rf)
    data_regress_rf_cat <- microbiota_tbl_rf %>% 
                           select(rf_cat_top20,  
                                   SampleID_NPA) %>%   # Don't change the order
                           inner_join(., metadata_pam, by="SampleID_NPA")  
    #colnames(data_regress_rf_cat) # 1-20 columns should be genera for the regression loop
  ## rf -- cont
    data_regress_rf_cont <- microbiota_tbl_rf %>% 
                            select(rf_cont_top20,  
                                   SampleID_NPA) %>%   # Don't change the order
                            inner_join(., metadata_pam, by="SampleID_NPA")  
    #colnames(data_regress_rf_cont) # 1-20 columns should be genera for the regression loop
  # Decide 1) loading and 2) dataset to regress below
    loading <- rf_cont_top20
    data_regress <-data_regress_rf_cont
```

##18.1. Linear correlation between total 25OHD level and each of the 20 genera selected from rf 
```{r Linear regression}
        coef_lm    <- numeric(length(loading))
        se_lm      <- numeric(length(loading))
        p.value_lm <- numeric(length(loading))
  for (i in seq_along(loading)){
       fit <- lm(vitd25_ngml ~ data_regress[[i]]
                 #+ Agecat2 
                 #+ female  
                 #+ raceethn 
                 #+ premature37
                 ...
                 #+ RSV + RV  #+ vitd25_ngml
                 , 
                 data=data_regress)  #1-20 are metabolites
  ...
  output_lm; 
  output_lm %>% filter(., p.value_lm <0.05) 
```

##18.2. Multivariable logistric regression adjusted for potential confounders 
```{r}
        est_logit <- numeric(length(loading))
        se_logit  <- numeric(length(loading))
        p.value_logit <- numeric(length(loading))
for (i in seq_along(loading)){
    fit <- glm(CPAPintubate ~ data_regress[[i]] # IntensiveTreatment or CPAPintubate
                 + Agecat2 
                 + female  
                 + raceethn 
                 + premature37
...
##18.3. Rondom-effects models adjusting for the covariates above and accounting for potential patient clustering within the hospitals.
```{r Random-effects}
      library("lme4"); 
  ...
  for (i in seq_along(loading)){
        fit2 <- glmer(IntensiveTreatment ~ data_regress[[i]] # IntensiveTreatment or CPAPintubate
                 + Agecat2  
                 + female 
                 + raceethn 
                 + premature37
